...
export const POST = withPremiumCheck(async function(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!foodData || !foodData.name) {
      return NextResponse.json(
        { error: 'Missing required fields: foodData with name' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      ...
      filename: `food-${foodData.name.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`
    });

  } catch (error) {
    console.error('Error processing food PDF request:', error);
